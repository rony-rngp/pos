...
<div class="row invoice-info" style="width:100%;margin-top: -25px">
    <p style="text-align: center;"><i>Cradit Customer Report ( {{  $dt->format('j-F-Y') }} )</i></p>
</div>
<!-- Table row -->
<div class="row">
    <div class="col-12 table-responsive " style="width: 100%">
        <table>
            ...
            @php
                ...
                $total_sum = 0;
            @endphp
            @foreach($credit_customer as $row)
                <tr class="text-center">
                    <td>{{ $sl++ }}</td>
                    <td>{{ $row->customer->name }} ({{$row->customer->mobile}} - {{$row->customer->address}}) </td>
                    <td>#{{ $row->invoice->invoice_no }} </td>
                    <td>{{ date('d-m-Y', strtotime($row->invoice->date)) }} </td>
                    <td>{{ $row->due_amount }}</td>
                    @php
                        $total_sum += $row->due_amount;
                    @endphp
                </tr>
            @endforeach
            <tr>
